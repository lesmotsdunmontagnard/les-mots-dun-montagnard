...
export function selectPoems(entries,params) {
 ...
 const result=entries.filter(e=>(!params.get('theme')||e.themes.includes(params.get('theme')))&&matches(e.lieu,params.get('lieu'))&&matches(e.date?.slice(0,7),params.get('mois'))&&norm([e.titre,e.texte,e.auteur,e.lieu].join(' ')).includes(query));
