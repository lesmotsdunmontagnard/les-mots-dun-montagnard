...
export function metadata(e,prefix='') {
 ...
 const parts=[];
 if(e.auteur)parts.push('Auteur : '+link('auteur',e.auteur,e.auteur));
...
export function selectPoems(entries,params) {
 ...
 const result=entries.filter(e=>(!params.get('theme')||e.themes.includes(params.get('theme')))&&matches(e.auteur,params.get('auteur'))&&matches(e.lieu,params.get('lieu'))&&matches(e.date?.slice(0,7),params.get('mois'))&&norm([e.titre,e.texte,e.auteur,e.lieu].join(' ')).includes(query));
 const byTitle=(a,b)=>a.titre.localeCompare(b.titre,'fr');
 const mode=params.get('tri')||'recent';
 return result.sort((a,b)=>mode==='titre'?byTitle(a,b):mode==='auteur'||mode==='lieu'?(a[mode]||'\uffff').localeCompare(b[mode]||'\uffff','fr')||byTitle(a,b):!a.date&&!b.date?byTitle(a,b):!a.date?1:!b.date?-1:(mode==='ancien'?a.date.localeCompare(b.date):b.date.localeCompare(a.date))||byTitle(a,b));
}
